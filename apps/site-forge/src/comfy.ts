...
import { fetchWithTimeout, UA, randomSeed } from "./util.ts";
...
export class Comfy {
  ...
  constructor(url?: string) {
    ...
    this.clientId = "site-forge-" + randomSeed().toString(36);
  }
  ...
  /** Upload a local image for LoadImage nodes; returns the name ComfyUI stored. */
  async uploadImage(filePath: string): Promise<string> {
    ...
    const r = await fetchWithTimeout(`${this.url}/upload/image`, { method: "POST", body: form, timeoutMs: 120000, headers: { "User-Agent": UA } });
    if (!r.ok) throw new Error(`upload failed ${r.status}: ${await r.text()}`);
    const j: any = await r.json();
    return j.name ?? name;
  }

  /** Queue an API-format graph. Throws with node-level errors when validation fails. */
  ...
  /** Poll history until the job completes or errors. */
  async wait(promptId: string, timeoutMs = 30 * 60 * 1000, onTick?: (elapsed: number) => void): Promise<QueueResult> {
    const start = Date.now();
    let lastTick = -1;
    while (Date.now() - start < timeoutMs) {
      const r = await fetchWithTimeout(`${this.url}/history/${promptId}`, { timeoutMs: 10000 });
      if (r.status === 404) {
        // not finished yet — history entry appears on completion
        await sleep(1500);
        const el = Math.round((Date.now() - start) / 1000);
        if (onTick && el !== lastTick) { onTick(el); lastTick = el; }
        continue;
      }
      ...
      if (status === "success" || Object.keys(entry.outputs ?? {}).length) {
        return { promptId, outputs: collectOutputs(entry), elapsedSec: Math.round((Date.now() - start) / 1000) };
      }
      await sleep(1500);
    }
    throw new Error(`timed out after ${Math.round(timeoutMs / 60000)} min waiting for ${promptId}`);
  }
  ...
  async download(f: ComfyFile): Promise<Buffer> {
    ...
    const r = await fetchWithTimeout(`${this.url}/view?${q}`, { timeoutMs: 120000 });
...
function collectOutputs(entry: any): ComfyFile[] {
  ...
  for (const nodeOut of Object.values<any>(entry.outputs ?? {})) {
    const push = (arr: any[] | undefined, kind: ComfyFile["kind"]) => {
      for (const f of arr ?? []) {
        if (f?.filename) out.push({ filename: f.filename, subfolder: f.subfolder ?? "", type: f.type ?? "output", kind });
      }
    };
...
/**
 * Substitute {{vars}} into a workflow graph (raw JSON text is fine — files on disk
 * are not valid JSON until substitution, because numeric placeholders are unquoted).
 * Numeric-looking values for known-numeric keys replace bare, everything else is
...
export function interpolate(raw: string | Record<string, any>, vars: Record<string, unknown>): { graph: Record<string, any>; missing: string[] } {
  ...
  const NUMERIC_KEYS = ["seed", "width", "height", "steps", "cfg", "shift", "length", "fps", "batch_size", "frame_rate", "strength", "max_shift", "base_shift", "terminal", "noise_seed"];
