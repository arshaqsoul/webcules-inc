...
- **One-click WhatsApp:** the "WhatsApp pitch" button uses the `whatsapp://send` protocol — it opens the WhatsApp desktop app straight to the chat with the message typed in. First click, the browser asks "Open WhatsApp?" — allow it (tick "remember"). After sending the hook, follow up with `before-after.jpg` as an image message.
